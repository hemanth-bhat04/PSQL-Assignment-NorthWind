...
SELECT 
    s."ShipperID",
    ROUND(
        (COUNT(CASE WHEN o."ShippedDate" > o."RequiredDate" THEN 1 END) * 100.0) / COUNT(o."OrderID"), 
        2
...
SELECT 
    ...
    ROUND(AVG(p."UnitPrice")::numeric,2) AS avg_unit_price,
    MIN(p."UnitPrice") AS min_unit_price,
    MAX(p."UnitPrice") AS max_unit_price,
    SUM(od."Quantity") AS total_units_on_order
FROM 
    products p
JOIN 
...
--query 4--
SELECT 
    TO_CHAR(o."OrderDate", 'YYYY-MM') AS month_year,  
    SUM(od."UnitPrice" * od."Quantity") AS total_sales_value
FROM 
    orders o
JOIN 
